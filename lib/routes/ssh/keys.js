...
function getKeys (req, res) {
	...
	.select('_id name public_key installed installing')
	.sort('-installed').exec(function (err, keys) {
		if (err) throw err;
...
function addKey (req, res) {
	...
	if (typeof key === 'undefined' || !key || 
		!key.name || key.name.length === 0) {
		errs.push("Name Invalid");
	}
	if (typeof key === 'undefined' || !key || 
		!key.public_key || key.public_key.length === 0) {

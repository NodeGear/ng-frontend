...
function createTicket (req, res) {
	...
	if (ticket.app) {
		...
		models.App.findOne({
			...
			cb(req, res, ticket, errs, app._id);
		});
	} else {
		cb(req, res, ticket, errs)
	}
}
...
function createCallback (req, res, ticket, errs, app) {
	...
	config.transport.sendMail({
		...
		to: "Alan Campbell <alan.campbell@castawaylabs.com>, Matej Kramny <matej@nodegear.com>",
		subject: sub,
		html: "\

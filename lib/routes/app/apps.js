...
function doAddApp (req, res) {
	...
	if (!template || template.length === 0) {
		...
		//	(template.command.length == 0 || template.image.length == 0)) {
		//}

		/*template.name != 'docker' && */
		if (!found || (template.location.length === 0 ||
			template.branch.length === 0)) {
			errs.push("Template Location or Branch Invalid");
		}
	...
	if (errs.length) {
		config.metrics.increment('frontend.app.add.failed');
	...
	// try to guess available url names.
	// Goes up to ten, if not available it uses the app id.
	var attempt = 0;
	...
	async.retry(10, function (cb, results) {
		...
		if (err) {
			// Fallback to _id (which is unique).
			app.nameUrl = app._id.toString();
		}

		config.metrics.increment('frontend.app.add.created.'+app.docker.image);

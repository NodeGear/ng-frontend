...
exports.httpRouter = function (app) {
	app.get('/app/:id/logs/:pid/:lid/download', getProcess,
		getLogId, downloadLog);
};
...
exports.router = function (app) {
	app.get('/app/:id/logs/:pid', getProcess, getLogs)
		...
		.get('/app/:id/logs/:pid/:lid/download', getLogId,
			downloadLog);
};
...
function getLog (req, res) {
	app.backend.lrange('pm:app_process_log_'+res.locals.lid, 0, 100,
...
function downloadLog (req, res) {
	app.backend.lrange('pm:app_process_log_'+res.locals.lid, 0, -1,
		function (err, entries) {
		if (err) throw err;
		...
		res.set({
			...
			'Content-Disposition': 'attachment; filename="'+
				res.locals.app.nameUrl+'-'+res.locals.lid+'.log"'
		});

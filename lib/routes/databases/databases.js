...
var mongoose = require('mongoose'),
	...
	admin_mysql = require('mysql')
		.createConnection(config.credentials.admin_mysql);

exports.admin_mysql = admin_mysql;

function admin_mysql_err (err) {
	bugsnag.notify(err, {
	...
	exports.admin_mysql = admin_mysql = mysql.createConnection(config.credentials.admin_mysql);
	admin_mysql.on('error', admin_mysql_err);
}
...
function createMongodbDatabase (req, database, db_pass, cb) {
	// Create mongodb db
	var db = new mongodb.Db('admin',
		new mongodb.Server(config.credentials.admin_mongodb.host,
			27017), {
		safe: false
	});
	...
	async.waterfall([
		...
		function authenticate (db, done) {
			if (config.credentials.admin_mongodb.user.length == 0) {
				return done(null, db);
			}

			db.authenticate(config.credentials.admin_mongodb.user,
				config.credentials.admin_mongodb.pass, function (err) {
				if (err) {
					return done(err);
				}
...
function createMysqlDatabase (req, database, db_pass, cb) {
	// Create mysql db
	...
	admin_mysql.query(query, function (err, results) {
		...
		query = 'GRANT ALL ON `'+database._id+'`.* to \''+uid+'\'@\'%\''+
...
function addDatabase (req, res) {
	...
	if (typeof db === 'undefined' || !db || !db.database_type ||
		db.database_type.length === 0 || !(db.database_type == 'mysql' ||
			db.database_type == 'mongodb' || db.database_type == 'mongolab')) {
	...
	var complete = function (err, details) {
		...
		if (database.database_type == 'mongodb') {
			database.url = 'mongodb://'+database.db_user+':'+database.db_pass+'@'+
				database.db_host+':27017/'+database.db_name;
		}
	...
	if (database.database_type == 'mongolab') {
		...
		database.createPassword(function (db_pass) {
			var fn = null;
			if (database.database_type == 'mongodb') {
				fn = createMongodbDatabase;
			} else if (database.database_type == 'mysql') {
				fn = createMysqlDatabase;
			}

			fn(req, database, db_pass, function (err, data) {
				complete(err, data);
			});
		});
	}

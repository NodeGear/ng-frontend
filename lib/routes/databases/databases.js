var mongoose = require('mongoose')
	...
	, mysql = require('mysql')
	, admin_mysql = mysql.createConnection(config.credentials.admin_mysql)
	...
	if (database.database_type == 'mongolab') {
		...
		database.createPassword(function(db_pass) {
			...
			if (database.database_type == 'mysql') {
				models.Database.findOne({
					user: req.user._id,
					database_type: 'mysql'
				}).select('db_pass').exec(function (err, database_old) {
					if (database_old != null) {
						db_pass = database_old.db_pass;
						database.db_pass = database_old.db_pass;
					}

					// Create mysql db
					var uid = req.user._id.toString();
					uid = (req.user._id.getTimestamp().getTime() / 1000 + '') + uid.substring(uid.length - 6);

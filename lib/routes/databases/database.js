...
function saveDatabase (req, res) {
	if (req.body.database && req.body.database.name &&
		req.body.database.name.length > 0) {
...
function redirectSSO (req, res) {
	...
	databases.getMongoLabCustomer(req, res, function (err, user) {
		...
		shasum.update(user.name + ':' + databases.mongolabSSOSalt +
			':' + timestamp);
		
		var token = shasum.digest('hex');

		res.redirect('https://mongolab.com/login/partners/nodegear/accounts/'+
			user.name+'?timestamp='+timestamp+'&token='+token);
	});
}
...
function deleteDatabase (req, res) {
	...
	if (db.database_type == 'mongolab') {
		databases.getMongoLabCustomer(req, res, function (err, user) {
			request({
				url: databases.mongolabEndpoint+'accounts/'+user.name+
	...
	if (db.database_type == 'mysql') {
		var query = 'REVOKE ALL PRIVILEGES ON `'+db._id.toString()+'`.* FROM \''+
			db.db_user+'\'@\'%\';';
	...
	if (db.database_type == 'mongodb') {
		var mongo_db = new mongodb.Db('admin',
			new mongodb.Server(config.credentials.admin_mongodb.host, 27017), {
			safe: false
		});

		mongo_db.open(function(err, mongo_db) {
			mongo_db.authenticate(config.credentials.admin_mongodb.user,
				config.credentials.admin_mongodb.pass, function (err, result) {
				if (err) {
					return bugsnag.notify(err, {
				...
				var newdb = mongo_db.mongo_db(res.locals.database._id.toString());

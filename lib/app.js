...
var express = require('express'),
	...
	backend = redis.createClient(
		config.credentials.redis_port, config.credentials.redis_host
	),
...
app.use(function(req, res, next) {
	...
	res.locals.loggedIn = typeof res.locals.user === 'object';
	res.locals.requiresTFA = false;
	if (res.locals.loggedIn) {
		res.locals.requiresTFA = req.user.tfa_enabled &&
			req.session.confirmedTFA !== true;
		res.locals.loggedIn = !(
			res.locals.requiresTFA ||
			!req.user.email_verified ||
			req.session.passwordUpdateRequired
		);
	}

	if (req.user) {
		config.metrics.set('frontend.active.users', req.user._id);
		...
		config.metrics.set('frontend.active.guests', req.ip);
	}
...
var socket_connections = 0;
io.on('connection', function (socket) {
	config.metrics.gauge('frontend.websockets.connections', ++socket_connections);
	console.log('Connected socket. Now', socket_connections)
	
	socket.on('disconnect', function () {
		config.metrics.gauge('frontend.websockets.connections', --socket_connections);
		console.log('disconnected socket. Now', socket_connections)
	});
});

	...
	for (var o in map) {
		...
		if (route.params) {
			for (var p in route.params) {
				...
				var fn = route.params[p];
				if (typeof fn === 'string') fn = obj[route.params[i]];
		...
		if (route.call) {
			var fn;
			if (typeof route.call === 'function') fn = route.call;
			else if (typeof route.call === 'string') fn = obj[route.call];
